...
def drawLeftInfo(frame,fpsText,mode="Reg",detector='haar',person=1, count=1):
    ...
    # 帧率
    cv2.putText(frame, "FPS:  " + str(round(fpsText,2)), (15, 30), cv2.FONT_ITALIC, 0.8, (0, 255, 0), 2)

    # 模式：注册、识别
    cv2.putText(frame, "Mode:  " + str(mode), (15, 60), cv2.FONT_ITALIC, 0.8, (0, 255, 0), 2)

    if mode == 'Recog':
        # 检测器
        cv2.putText(frame, "Detector:  " + detector, (15, 90), cv2.FONT_ITALIC, 0.8, (0, 255, 0), 2)

        # 人数
        cv2.putText(frame, "Person:  " + str(person), (15, 120), cv2.FONT_ITALIC, 0.8, (0, 255, 0), 2)

        # 总人数
        cv2.putText(frame, "Count:  " + str(count), (15, 150), cv2.FONT_ITALIC,0.8, (0, 255, 0), 2)
...
# 人脸识别
def faceRecognize(detector='haar',threshold=0.4, write_video = False,resize_w=700,resize_h=400):
...
parser.add_argument("--name", type=str, default='wufeng',
                    help="人脸姓名，英文格式")                       
